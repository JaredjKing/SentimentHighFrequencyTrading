...
class TestFinanceSentiment(unittest.TestCase):
    def test_analyze_finbert_positive1(self):
    ...
    def test_analyze_finbert_positive2(self):
        text = """Market P/Es are very low and quarterly reports are very strong"""
        result = analyze_finbert(text)
        self.assertIsInstance(result, float)
        self.assertGreater(result, 0.999)

    def test_analyze_finbert_negative(self):
        text = """the market is looking grim and bearish"""
        result = analyze_finbert(text)
        self.assertIsInstance(result, float)
        self.assertLess(result, -0.5)

    def test_analyze_finbert_neutral(self):
        text = """President Xi Jinping says China wants to be friends with the US and that his nation won't fight a war with anyone, one of his clearest remarks 
        yet proclaiming a desire for peaceful ties between the world's two largest economies"""
        result = analyze_finbert(text)
        self.assertIsInstance(result, float)
